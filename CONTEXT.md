...
| **AudioApi** | The SFX/haptics contract: `pickup`, `place`, `reject`, `clear`, `combo`, `perfect`, `gameOver` event methods plus `setMuted` / `isMuted` / `unlock`. Two implementations: `createAudio()` plays MP3s from `/sounds/*.mp3` and vibrates via `navigator.vibrate`; `createSilentAudio()` is a no-op used in tests. |
...
| **unlock** | A one-shot priming call to `AudioApi.unlock()` made inside the first user-gesture (pointerdown) on the page. Required because iOS Safari blocks audio playback until a gesture occurs; `unlock` plays + pauses each clip so subsequent SFX plays are uninterrupted. |
