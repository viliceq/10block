...
| **Layout** | `computeLayout`'s return: `{ orientation, cellSize, boardSize, trayCellSize, traySlotSize, overflow }`, all sizes integer px. `boardSize = 2·boardPad + 10·cellSize + 9·gap`; spare pixels are even outer margin. `overflow` ⇒ the page may scroll as a last resort (board still never clipped). The binder writes these onto `:root`. |
